...
toRankDocs :: RawDocIndex PackageInfo -> [(URI, RawDoc RankDescr, Maybe Float)]
toRankDocs (RDX sm)
  = map toRank . M.toList $ sm

toRank :: (URI, RawDoc PackageInfo) -> (URI, ([a], String, Maybe RankDescr), Maybe Float)
toRank (url, (_rawCxs, _rawTitle, pinfo))
  = (url, ([], "", Nothing), fmap p_rank pinfo)
-- toRank d = (uri d, ([], "", Nothing), fmap p_rank $ custom d)

rankToCommand :: Bool -> UTCTime -> RawDocIndex PackageInfo -> Command
